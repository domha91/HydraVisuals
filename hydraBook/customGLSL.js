...
SETFUNCTION
// A custom GLSL function can be defined using setFunction(). The structure follows
// the format of builtin functions. Defining a custom GLSL function does not mean
// that you are allowed to design an arbitrary function; the function has to have
// specific inputs and an output, based on its type. The types are src color
// combine combineCoords.

CHROMA KEY
// This example modifies color to replace green background with
// transparency (i.e., chroma keying). The GLSL code is ported from Inigo Quilez's example.
...
RAY MARCHING
// In this example, a simple ray marching is implemented as a coord function,
// which literally modifies the coordinates, to map incoming texture to a sphere.
...
// A noise input can be added as an argument to displace the sphere surface.
// This can be done with combineCoord type, which is used by modulate* family.
